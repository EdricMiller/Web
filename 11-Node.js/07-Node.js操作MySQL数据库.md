...
[{"id":1,"name":"admin","age":28},{"id":2,"name":"许嵩","age":34},{"id":3,"name":"邓紫棋","age":28}]
...
const strSql5 = 'CREATE TABLE `admin_table5` (`id` int NOT NULL AUTO_INCREMENT,`name` varchar(255) DEFAULT NULL,`age` int DEFAULT NULL,PRIMARY KEY (`id`));';
...
connection.query(strSql5, (err, result) => {
    if (err) {
        ...
        // 新建表成功
        console.log('admin 新建表成功：' + JSON.stringify(result));
    }
})
...
```js
// 在指定的表中插入数据
const strSql6 = "insert into admin_table5 (name, age) values ('admin', '28')";
...
connection.query(strSql6, (err, result) => {
    if (err) {
        ...
        // 在指定的表中插入数据成功
        console.log('admin 在指定的表中插入数据成功：' + JSON.stringify(result));
    }
});
...
```js
// 在指定的表中插入数据（数据作为变量）
const strSql7 = "insert into admin_table5 (name, age) values (?, ?)";
