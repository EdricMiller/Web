...
{
    //两个变量时
    let [a, b] = ['admin']; //a 赋值为：admin。b没有赋值
    console.log(a + ',' + b); //输出结果：admin,undefined
}

{
    //两个变量时
    let [a, b = 'admin'] = ['admin']; //a 赋值为：admin。b 采用默认值 admin
    console.log(a + ',' + b); //输出结果：admin,admin
}
```
...
```javascript
{
    let [a, b = 'admin'] = ['admin', undefined]; //b 虽然被赋值为 undefined，但是 b 会采用默认值
    console.log(a + ',' + b); //输出结果：admin,admin
}

{
    let [a, b = 'admin'] = ['admin', null]; //b 被赋值为 null
    console.log(a + ',' + b); //输出结果：admin,null
}
```
...
```js
const person = { name: 'admin', age: 28, sex: '男' };
...
```js
const person = { name: 'admin', age: 28 };
let { name: myName, age: myAge } = person; // 对象的结构赋值

console.log(myName); // 打印结果：admin
